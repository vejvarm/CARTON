...
def index_one(item, mock_data=False):
    if mock_data:
        kg_types = {}
    id, label = item
    for i in range(10):
        try:
            CLIENT.index(index='csqa_wikidata', id=id, document={'id': id,
...
def index_mock_entry(index):
    ...
    CLIENT.index(index=index, id=id, document={'label': label, 'type': type})  # TODO: Question: can there be more types for one entity?
...
def index_mock_label_type_entries(index):
    ...
    types = [['Q911', 'Q1337'], ['Q911'], ['Q911'], ['Q888', 'Q911'], ['Q1337'], ['Q999', 'Q1337'], ['Q999'], ['Q888'], ['Q888'], ['Q911']]

    for id, lab, tp in zip(ids, labels, types):
        CLIENT.index(index=index, id=id, document={'label': lab, 'types': tp})


def fill_ent_index(index=args.elastic_index_ent, source_json='index_ent_dict.json'):
...
def fill_rel_index(index=args.elastic_index_rel, source_json='index_rel_dict.json'):
    ...
    for rid, rlabel in index_dict.items():
...
def fill_rdf_index(index=args.elastic_index_rdf, source_json='index_rdf_dict.json'):
    ...
    for sid, rel_dict in index_dict.items():
        for rid, object_list in rel_dict.items():
            for oid in object_list:
                _id = str(hash(f'{sid}{rid}{oid}'))
